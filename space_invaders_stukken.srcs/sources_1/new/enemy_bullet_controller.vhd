...
entity enemy_bullet_controller is
    Port (
        ...
        collision_detected : in STD_LOGIC; -- NIEUW: Input om kogel te killen
...
begin
    ...
    begin
        if reset = '1' then
            ...
            -- 0. COLLISION CHECK (Hoogste prioriteit)
            -- Als Game Logic zegt dat we geraakt hebben, zet kogel uit.
            if collision_detected = '1' then
                active <= '0';
                -- Optioneel: by resetten, maar active=0 is genoeg voor de renderer
            end if;
            ...
            -- 2. GAME LOOP (Timer)
            cnt <= cnt + 1;
            if cnt = 416667 then -- 60 Hz
                cnt <= (others => '0');
                
                -- Is er een kogel actief? BEWEGEN
                if active = '1' then
                    if by < 475 then 
                        by <= by + SPEED;
                    else
                        active <= '0'; -- Reset kogel
                        by <= 0;
                    end if;
                    
                -- Geen kogel actief? Check of we MOETEN schieten
